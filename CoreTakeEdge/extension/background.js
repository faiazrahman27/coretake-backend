chrome.action.onClicked.addListener(async (tab) => {
  try {
    ...
    if (!response || !response.text) {
      console.error("Could not read page text. Some sites block script injection.");
      return;
    }
    ...
    // Send to backend
    const backendResponse = await fetch("https://coretake-backend.onrender.com/summarize", {
      method: "POST",
      ...
      url: "data:text/html," + encodeURIComponent(`
        ...
        <head>
          ...
          <style>
            ...
            h1 { color: #2c3e50; }
            p { background: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.1); white-space: pre-wrap; }
          </style>
        </head>
        <body>
          <h1>Page Summary</h1>
